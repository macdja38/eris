...
class VoiceConnection extends EventEmitter {
    constructor(id, options) {
        ...
        this.wsSendError = this.wsSendError.bind(this);
        ...
        if(!options.shared) {
            ...
            this.piper.on("warn", (w) => this.emit("warn", w));
            this.piper.on("debug", (d) => this.emit("debug", d));
    ...
    disconnect(error, reconnecting) {
        ...
        if(reconnecting) {
            ...
            this.stopPlaying("disconnecting");
        }
        if(this.heartbeatInterval) {
            clearInterval(this.heartbeatInterval);
    ...
    */
    ...
    _send() {
        if(!this.piper.encoding && this.piper.dataPacketCount === 0) {
            return this.stopPlaying("not encoding and packet count 0");
        }
    ...
    */
    ...
    sendWS(op, data) {
        if(this.ws && this.ws.readyState === WebSocket.OPEN) {
            ...
            this.ws.send(data, this.wsSendError);
    ...
    wsSendError(err) {
      if(this.ready) {
        this.emit("warn", `Voice WS send error` + (err && " | " + err));
        this.disconnect(err, false);
      } else {
