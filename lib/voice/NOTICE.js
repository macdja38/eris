...
class WolkeStream {
  ...
  processRes(req, res) {
    ...
    req.on('error', (err) => {
      if (!err) return;
      console.error("wolkeStream - reqError", err);
    ...
    res.on('aborted', (err) => {
      if (!err) return;
      console.error("wolkeStream - aborted", err);
      this.output.pause();
      res.unpipe();
      res.removeAllListeners();
      req.removeAllListeners();
    ...
    res.on('error', (err) => {
      if (!err) return;
      console.error("wolkeStream - resError", err);
      if (err.message === 'read ECONNRESET') {
        this.output.pause();
        res.unpipe();
        res.removeAllListeners();
...
/*
 ...
 This file is licenced by Wolke / Julian under the following terms.
 ...
 Copyright {2016} {Wolke/Julian Hannig}
