...
const HTTPS = require("stream-resume");
// const wolkeStream = require("./NOTICE.js");
...
function createHttpGetter(http) {
    return function getHttps(link, callback, errorCallback, depth = 8) {
        http.get(link, (res) => {
            ...
            if (statusCode === 301 || statusCode === 302 || statusCode === 307 || statusCode === 308) {
                ...
                if (depth <= 0) {
                    ...
                    getHttps(headers.location, callback, errorCallback,depth - 1);
                }
            } else {
                callback(res);
            }
...
const httpGet = createHttpGetter(HTTP);
const httpsGet = createHttpGetter(HTTPS);
...
class Piper extends EventEmitter {
    constructor(converterCommand, opus) {
        ...
        this._dataPackets = [];
        this._dataPacketMax = 180;
        this._dataPacketMin = 60;
        this.encoding = false;
        this.libopus = true;
        this.opus = opus;
        this.volumeLevel = 1;
    ...
    encode(source, options) {
            ...
            if(options.format === "dca" || options.format === "ogg" || options.format === "webm" || options.format === "pcm") {
                if(source.startsWith("http://") || source.startsWith("https://")) {
                    ...
                    if(source.startsWith("http://")) {
                        httpGet(source, res => res.pipe(passThrough), (e) => this.stop(e));
                    } else {
                        httpsGet(source, res => res.pipe(passThrough), (e) => this.stop(e));
                    }
                } else {
                    try {
                        FS.statSync(source);
                        ...
                        if(err.code === "ENOENT") {
                            ...
                            this.emit("error", new Error("An error occurred trying to access that file."));
                        }
                        return false;
                    }
        ...
        if(options.format === "dca") {
            ...
            this.streams.push(source.pipe(new WebmOpusTransformer()).once("error", (e) => this.stop(e).on("warn", (w) => this.emit("warn", w)).on("debug", (d) => this.emit("debug", d))));
        ...
        this._endStream.on("warn", (w) => this.emit("warn", w));
        this._endStream.on("debug", (d) => this.emit("debug", d));
    ...
    stop(e, source) {
        ...
        if(e) {
            ...
            this.emit("warn", "stop from source: " + source ? util.inspect(source) : source);
        }
