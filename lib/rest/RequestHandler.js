...
class RequestHandler {
    ...
    static responseToErisError(req, resp, method, url, response) {
        var possibleCode;
        try {
            var json = JSON.parse(response);
            if (json.code !== 0) {
                possibleCode = Errors.getDiscordResponse(json.code);
                if (json.message) {
                    return new Errors.ErisError(possibleCode, json.message);
                } else {
                    return new Errors.ErisError(possibleCode);
                }
            }
        } catch (error) {
            if (!error.message.indexOf("SyntaxError: Unexpected token < in JSON at position" < -1)) {
                throw error;
            }
        }

        if (resp.statusCode) {
            possibleCode = Errors.getDiscordHTTPResponse(resp.statusCode);
            return new Errors.ErisError(possibleCode, resp.statusCode, req.method, req.path);
        }

        return new Errors.ErisError("ERISERR_INVALID_STATUS_CODE");
    ...
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            ...
            var actualCall = (cb) => {
                ...
                try {
                    ...
                    if(file) {
                        if(Array.isArray(file)) {
                            ...
                            throw new Errors.ErisError("ERISERR_INVALID_FILE");
                        }
                    } else if(body) {
                ...
                req.once("abort", () => {
                    cb();
                    reqError = reqError || new Errors.ErisError("ERISERR_REQUEST_ABORTED_BY_SERVER", method, url);
                    reqError.req = req;
                    reject(reqError);
                }).once("aborted", () => {
                    cb();
                ...
                req.once("response", (resp) => {
                    ...
                    if(this.latencyRef.lastTimeOffsetCheck < Date.now() - 5000) {
                        ...
                        if(this.latencyRef.timeOffset - this.latencyRef.latency >= this._client.options.latencyThreshold && timeOffset - this.latencyRef.latency >= this._client.options.latencyThreshold) {
                            this._client.emit("warn", new Errors.ErisError("ERISERR_CLOCK_DESYNC", this.latencyRef.timeOffset));
                        }
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.statusCode >= 300) {
                            ...
                            cb();
                            reject(RequestHandler.responseToErisError(req, resp, method, url, response));
                            return;
                        }
                ...
                req.setTimeout(15000, function() {
                    reqError = new Errors.ErisError("ERISERR_REQUEST_TIMEOUT", method, url);
                    req.abort();
                });
