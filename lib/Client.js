...
class Client extends EventEmitter {
    ...
    * @arg {Number} [options.workerPort=8080] The port voice workers should connect on
    */
    constructor(token, options) {
        ...
        if (this.options.workerPort) {
            this.workerWSS = new WebSocket.Server({port: this.options.workerPort});

            this.workerWSS.on("connection", (ws) => {
                this.workerWS = ws;
                this.workerWS.sendWS = (json) => {
                    ws.send(JSON.stringify(json));
                };
                this.workerWS.on('message', (data) => {
                    ...
                    switch (data.action) {
                        case "SEND_WS": {
                            ...
                            console.log(message);
                            this.shards.get(this.guildShardMap[data.guild_id] || 0).sendWS(message.op, message.d);
                        }
                    }
                });
            });
        ...
        this.joinPendingVoiceConnections = {};
        this.leavePendingVoiceConnections = {};
        this.voiceSessions = {};
    }
    ...
    */
    joinVoiceChannel(channelID, options) {
        ...
        var guildID = this.channelGuildMap[channelID];
        ...
        return new Promise((resolve, reject) => {
            this.joinPendingVoiceConnections[guildID] = {resolve, reject};
        });
    }
     ...
     * @private
     */
    _voiceServerUpdate(packet) {
        var data = {
            endpoint: packet.endpoint,
            guild_id: packet.guild_id,
            session_id: this.voiceSessions[packet.guild_id],
            token: packet.token,
        ...
        console.log(this.voiceSessions[packet.guild_id]);
        this.emit("VOICE_SERVER_UPDATE", data);
        if (this.joinPendingVoiceConnections[packet.guild_id]) {
            this.joinPendingVoiceConnections[packet.guild_id].resolve();
        }
    }

    _voiceChannelUpdate(packet) {
        // this.emit("voiceChannelUpdate", packet);
        this.voiceSessions[packet.guild_id] = packet.session_id;
        console.log("voiceChannelUpdate", packet);
    }
    ...
    */
    leaveVoiceChannel(channelID) {
        ...
        return new Promise((resolve, reject) => {
            this.leavePendingVoiceConnections[guildID] = {resolve, reject};
        });
    }

    play(channelID, url, options) {
        var channel = this.getChannel(channelID);
        if(!channel) {
            return;
        }
    ...
    /**
     * Changes the volume of the guild's voice connection
     * @arg {String} guildID
     * @arg {Number} volume
     */
